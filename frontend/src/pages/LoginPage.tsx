...
export function LoginPage() {
  const { isAuthenticated, login: persistLogin } = useAuth();
  ...
  const submit = handleSubmit(async (values) => {
    try {
      ...
      // A mutation mantém o estado de erro usado pela mensagem visual.
    }
  });
    ...
    <main className='min-h-screen bg-[radial-gradient(circle_at_top_left,#ffedd5,transparent_28%),linear-gradient(135deg,#18181b_0%,#27272a_44%,#f97316_100%)] px-4 py-8 text-white'>
      <div className='mx-auto grid min-h-[calc(100vh-4rem)] max-w-6xl items-center gap-8 lg:grid-cols-[0.9fr_1.1fr]'>
        <section className='space-y-8 text-left'>
          <div>
            <div className='text-xl font-semibold uppercase tracking-[0.24em] text-orange-200'>
              Pitang
            </div>
            ...
            <p className='mt-4 max-w-lg text-base text-zinc-200'>
              Acompanhe solicitações, aprovações e pagamentos em um ambiente
              operacional seguro.
            </p>
          </div>
          <div className='grid gap-3 sm:grid-cols-3'>
            {[
              {
                description: 'Cadastro com categoria, valor, data e anexos.',
                icon: FilePlus2,
                label: 'Solicitação',
              },
              {
                description: 'Aprovação ou rejeição com justificativa.',
                icon: ClipboardCheck,
                label: 'Análise',
              },
              {
                description: 'Financeiro marca aprovadas como pagas.',
              ...
              return (
                ...
                >
                  <div className='mb-2 inline-flex rounded-md bg-white/10 p-2 text-orange-200'>
          ...
          <CardContent className='p-5 sm:p-6'>
            <div className='mb-6 text-left'>
              ...
              <p className='mt-2 text-sm text-zinc-500 dark:text-zinc-400'>
                Use suas credenciais corporativas para acessar o sistema interno
                de reembolsos.
              </p>
            </div>
            ...
            <form onSubmit={submit} className='space-y-4'>
              ...
              <Button
                type='submit'
                disabled={loginMutation.isPending}
                className='w-full bg-zinc-950 hover:bg-zinc-800 dark:bg-orange-600 dark:hover:bg-orange-700'
              >
